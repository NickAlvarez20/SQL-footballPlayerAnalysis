...
SELECT MIN(pick_number)
    FROM picks;
/*HAVING Category*/
SELECT college, AVG(pick_number) AS college_pick FROM picks
GROUP BY college
HAVING college_pick < 28;

SELECT college, MAX(pick_number) AS college_pick FROM picks
GROUP BY college
HAVING college_pick < 28;

SELECT college, MIN(pick_number) AS college_pick FROM picks
GROUP BY college
HAVING college_pick < 28;

/*CASE*/
SELECT pick_number, college,
    CASE
    WHEN pick_number > 75 THEN "Bad"
    WHEN pick_number >50 THEN "Decent"
    WHEN pick_number > 25 THEN "Good"
    WHEN pick_number > 10 THEN "Great"
    ELSE "Best"
    END AS "Quality"
    FROM picks;
/*AND*/
SELECT * from picks WHERE pick_number < 25 AND college = "Oregon";
/*OR*/
SELECT * from picks WHERE pick_number < 25 OR pick_number > 75;
